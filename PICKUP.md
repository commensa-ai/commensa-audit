...
- **OSS-sweep caveat — explicit-signal over-counts rework on human-driven repos (S7, erpnext proof).** frappe/erpnext (150 newest PRs) scored 78% rework tax — but all 117 came from the `explicit` title signal firing on conventional-commit `fix:` titles + backports, with only 12.7% agent-marked, 4% of lines corrective, 0 churn clusters, 96.8% survival. The headline % is misleading for repos that aren't agent-driven. Before the OSS sweep posts big-repo numbers, gate the rework-tax headline on agent-marked share and/or add a "explicit-dominant + low-agent-marked" caveat to the report. 0.3.0+ classifier work.
