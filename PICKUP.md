...
## Current priority
**Matt's lane (his accounts, not a build session):** (1) **commit + tag v0.2.1 to git and push** — PyPI has 0.2.1 but `main` is still 0.2.0 (`bd8b6f8`); the fix + tests + BUILD_LOG + this PICKUP are uncommitted; (2) Day 3–4 posting per ../execution/tasks.md (flagship + Show HN, numbers re-verified). Build-side next: nothing queued — new work starts from a fresh design conversation or tasks.md.
